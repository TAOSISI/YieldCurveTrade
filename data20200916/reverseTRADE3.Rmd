...
#反转策略设置止损
止损平仓的方式主要是基于净值的回撤，当原净值曲线回撤超过一定比例后，进行平仓。然而这一方式无法在后续回复期间获得收益。假设按照原先的仓位继续持仓，当净值回复后在继续持仓。reverseTRADE3与reverseTRADE2的不同之处在于reverseTRADE3的历史分位数选择标准差的倍数。
...
for (i in c(2:length(data1$date_id))) {
  # i=28
  # 上一日仓位为空，且今日仓位不为0，说明今日为首日开仓,增加上一日仓位与今日持仓反向的情况
  if((data1$lag_position_T[i-1]==0 & data1$lag_position_T[i]!=0) | (data1$lag_position_T[i-1]*data1$lag_position_T[i]<0)){
    ...
    if(data1$lag_position_T[i-1]*data1$lag_position_T[i]>0){
      ...
      #if(data1$ismax[i] == 0){data1$MINJZ[i] = data1$XNJZ[i]}
      ...
      if(data1$Drawdowns[i-1]>zhisunxian & data1$FCSY[i-1]<fanchouxian){
          ...
          data1$lag_position_TF_ZS[i] = 0
            ...
            if((data1$FCSY[i-1]>=fanchouxian) & data1$lag_position_T_ZS[i-1]==0){

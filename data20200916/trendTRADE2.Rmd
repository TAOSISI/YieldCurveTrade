...
# 趋势策略进阶
当隐含收益率利差向上突破较高分位数时，预计未来利差将继续走阔，则当其回落至对应较高分位数后平仓。
当隐含收益率利差向下下穿较高分位数后，则认为其会均值回复，则此时预计未来利差将缩小，则到达中位数后则平仓。
```{r}
...
data1 <- as.data.frame(data1 %>% mutate(position_T90 = ifelse(FutYHret >= quan90, -1,ifelse(FutYHret <= quan10, 1, position_T90)),
                                        position_T80 = ifelse(FutYHret >= quan80, -1,ifelse(FutYHret <= quan20, 1, position_T80)),
                                        position_T70 = ifelse(FutYHret >= quan70, -1,ifelse(FutYHret <= quan30, 1, position_T70)),
                                        position_T60 = ifelse(FutYHret >= quan60, -1,ifelse(FutYHret <= quan40, 1, position_T60)),
                                        position_TF90 = ifelse(FutYHret >= quan90, 2,ifelse(FutYHret <= quan10, -2, position_TF90)),
                                        position_TF80 = ifelse(FutYHret >= quan80, 2,ifelse(FutYHret <= quan20, -2, position_TF80)),
                                        position_TF70 = ifelse(FutYHret >= quan70, 2,ifelse(FutYHret <= quan30, -2, position_TF70)),
                                        position_TF60 = ifelse(FutYHret >= quan60, 2,ifelse(FutYHret <= quan40, -2, position_TF60))))
```

```{r}
data1 <- as.data.frame(data1 %>% mutate(position_T = position_T70,position_TF = position_TF70))

...
for (i in c(2:length(data1$date_id))) {
  ...
  # 上一日仓位为空，且今日仓位不为0，说明今日为首日开仓
  if(data1$lag_position_T[i-1]==0 & data1$lag_position_T[i]!=0){
    # 首日开仓，今日账户金额为最大最小净值（修改：应该和开仓前资金相比较），由于这里最大值最小值和现值相同，所以比当前净值大还未出现，所以ismax为0
   ...
   data1$ismax[i] = 0
    ...
    # 前一日有仓位且今日也有仓位
    if(data1$lag_position_T[i-1]!=0 & data1$lag_position_T[i]!=0){
      # 如果前一日的最大净值小于当日的账户净值，那么当日账户净值位最大净值，最小净值为最大净值出现后的最小净值所以为当前账户净值，ismax为0

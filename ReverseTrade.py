    ...
    def Simulate_ZS(self,df,stop_point,start_point):
        ...
        for i in range(1,len(df['ZHJE'])):
            ...
            if (df['lag_position_T'][i-1] == 0) & (df['lag_position_T'][i] != 0): 
                ...
                df['XZZJ'][i] = (df['ZHJE'][i] - df['KCSS'][i] * max(2*0.012*df['TF_settle'][i]*10000,0.02*df['T_settle'][i]*10000) * abs(df['lag_position_T'][i]))/df['ZHJE'][i]
            
            else:
                if (df['lag_position_T'][i-1] != 0) & (df['lag_position_T'][i] != 0):  #从持仓到持仓
                    df['KCSS'][i] = df['KCSS'][i-1]
                    df['XNJZ'][i] = 10000*df['KCSS'][i]*df['lag_position_T'][i]*(df['T_settle'][i]-df['T_settle'][i-1])+\
                    10000*df['KCSS'][i]*df['lag_position_TF'][i]*(df['TF_settle'][i]-df['TF_settle'][i-1])+ df['XNJZ'][i-1]
                    df['ZHJE'][i] = df['XNJZ'][i]
                    ...
                    if (df['Drawdowns'][i-1] > stop_point ) & (df['FCSY'][i-1] < start_point ):  #满足平仓条件
                        ...
                        if(df['FCSY'][i-1]>=start_point) & (df['position_T_ZS'][i-1] == 0):   #若之前因Drawdown使得处于平仓状态，但FCSY[i-1]大于了0.2
                            #重新开仓
                            df['KCSS'][i] = math.floor(df['ZHJE'][i-1]/(max(2*0.012*df['TF_settle'][i-1]*10000,0.02*df['T_settle'][i-1]*10000))/4)
                            df['ZHJE'][i] = 10000*df['KCSS'][i]*df['lag_position_T'][i]*(df['T_settle'][i]-df['T_settle'][i-1])+\
                    10000*df['KCSS'][i]*df['lag_position_TF'][i]*(df['TF_settle'][i]-df['TF_settle'][i-1])+ df['ZHJE'][i-1]
...
def main():
    test = StepReverse('T_TFirret.csv','ADPFutureClose2020-09-16.csv','StepReverse.csv','ReverseTrade.csv',100,[90,80,70,60,50,40,30,20,10])

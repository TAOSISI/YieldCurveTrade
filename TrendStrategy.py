...
class TrendStrategy:
    def __init__(self, T_TFirret_path, ADPFutureClose_path, TrendStrategySave_path,StepTrendZS_savepath,days,quantiles):
        self.T_TFirret_path = T_TFirret_path
        self.ADPFutureClose_path = ADPFutureClose_path
        self.TrendStrategySave_path = TrendStrategySave_path
        self.StepTrendZS_savepath = StepTrendZS_savepath
        self.days = days
    ...
    def GetPosition(self):
        ...
        for quantile in self.quantiles:
            if quantile > 50:
                ...
                for i in range(len(df['date_id'])):
                    ...
                    if ((df['quan50'][i] < df['FutYHret'][i]< df['quan'+str(quantile)][i]) & begin01):
                    ...
                    if ((df['quan'+ str(100-quantile)][i]<df['FutYHret'][i] < df['quan50'][i]) & begin02) :
                        df['position_T'+ str(quantile)][i] = -1
                        df['position_TF'+ str(quantile)][i] = 2  
                ...
                df_test = pd.read_csv('TrendStrategy_T.csv', encoding='utf-8')
        ...
        #df.to_csv(self.TrendStrategySave_path,index = False)

        return df    

    def Simulate(self,quantile):
        df = self.GetPosition()

        df['position_T'] = df['position_T' + str(quantile)].copy()
        df['position_TF'] = df['position_TF' + str(quantile)].copy()
    ...
    def Simulate_ZS(self):
        ...
        df = self.GetPosition()
        df['position_T'] = df['position_T90'].copy()+df['position_T80'].copy()
        df['position_TF'] = df['position_TF90'].copy()+df['position_TF80'].copy()
            ...
            if (df['lag_position_T'][i-1] == 0) & (df['lag_position_T'][i] != 0):
                ...
                if(df['lag_position_T'][i-1] != 0) & (df['lag_position_T'][i] != 0):
                    if(df['MAXJZ'][i-1] <= df['ZHJE'][i]):
                        ...
                        df['ismax'][i] = 1
                    df['MINJZ'][i] = (min(df['MINJZ'][i-1],df['ZHJE'][i])) if (df['ismax'][i]==1) else df['ZHJE'][i] 
        ...
        for i in range(1,len(df['ZHJE'])):
            if (df['position_T_ZS'][i-1] == 0) & (df['position_T_ZS'][i] != 0): 
                df['KCSS'][i] = math.floor(df['ZHJE'][i-1]/(max(2*0.012*df['TF_settle'][i]*10000,0.02*df['T_settle'][i]*10000))/4)
                df['ZHJE'][i] = 10000*df['KCSS'][i]*df['position_T_ZS'][i]*(df['T_settle'][i]-df['T_settle'][i-1])+\
                    10000*df['KCSS'][i]*df['position_TF_ZS'][i]*(df['TF_settle'][i]-df['TF_settle'][i-1])+ df['ZHJE'][i-1]
                ...
                df['KCSS'][i] = 0
                df['ZHJE'][i] = df['ZHJE'][i-1]
            df['XZZJ'][i] = df['ZHJE'][i] - df['KCSS'][i] * max(2*0.012*df['TF_settle'][i]*10000,0.02*df['T_settle'][i]*10000) * abs(df['position_T_ZS'][i])
        
        df.to_csv(self.StepTrendZS_savepath,index = False)

def main():
    test = TrendStrategy('T_TFirret.csv','ADPFutureClose2020-09-16.csv','TrendStrategy.csv','StepTrendZS.csv',100,[90,80,50,20,10])
    #test.Simulate(80)
    test.Simulate_ZS()
